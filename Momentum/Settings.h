...
{
  if ((envTypeFilt >= -8) && (envTypeFilt <= 8))
    return envTypeFilt + 9;
  else
    return 9;
}
...
FLASHMEM void settingsAmpEnv(int index, const char *value)
{
  if (strcmp(value, "Lin") == 0)
    envTypeAmp = -128;
  else if (strcmp(value, "Exp -8") == 0)
    envTypeAmp = -8;
  else if (strcmp(value, "Exp -7") == 0)
    envTypeAmp = -7;
  else if (strcmp(value, "Exp -6") == 0)
    envTypeAmp = -6;
  else if (strcmp(value, "Exp -5") == 0)
    envTypeAmp = -5;
  else if (strcmp(value, "Exp -4") == 0)
    envTypeAmp = -4;
  else if (strcmp(value, "Exp -3") == 0)
    envTypeAmp = -3;
  else if (strcmp(value, "Exp -2") == 0)
    envTypeAmp = -2;
  else if (strcmp(value, "Exp -1") == 0)
    envTypeAmp = -1;
  else if (strcmp(value, "Exp 0") == 0)
    envTypeAmp = 0;
  else if (strcmp(value, "Exp +1") == 0)
    envTypeAmp = 1;
  else if (strcmp(value, "Exp +2") == 0)
...
FLASHMEM void settingsKeyTracking(int index, const char *value)
{
  ...
  if (strcmp(value, "Half") == 0)
    updateKeyTracking(0.5);
  if (strcmp(value, "Full") == 0)
    updateKeyTracking(1.0);
}
...
FLASHMEM void settingsMonophonic(int index, const char *value)
{
  uint8_t monophonic;
  if (strcmp(value, "Off") == 0)
    monophonic = MONOPHONIC_OFF;
  if (strcmp(value, "Last") == 0)
    monophonic = MONOPHONIC_LAST;
  if (strcmp(value, "First") == 0)
    monophonic = MONOPHONIC_FIRST;
  if (strcmp(value, "Highest") == 0)
    monophonic = MONOPHONIC_HIGHEST;
  if (strcmp(value, "Lowest") == 0)
    monophonic = MONOPHONIC_LOWEST;
  if (strcmp(value, "Legato") == 0)
    monophonic = MONOPHONIC_LEGATO;
...
{
  if (groupvec[activeGroupIndex]->getKeytrackingAmount() == 0.0f)
    return 0;
  if (groupvec[activeGroupIndex]->getKeytrackingAmount() == 0.5f)
    return 1;
  if (groupvec[activeGroupIndex]->getKeytrackingAmount() == 1.0f)
    return 2;
  return 0;
}
